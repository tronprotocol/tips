...
discussions-to: https://github.com/tronprotocol/tips/issues/831
status: Final
...
This TIP introduces a standard for NFTs with private metadata that addresses these requirements through privacy-preserving verification mechanisms, enabling secure ownership and transfer of valuable agent data while maintaining confidentiality and verifiability. This standard will serve as a foundation for the emerging agent ecosystem, allowing platforms to provide verifiable agent ownership and secure metadata management in a decentralized manner.

### Relationship to ERC-7662

Both [ERC-7662][erc7662] and this standard represent AI agents as NFTs, but they focus on different layers:

- **ERC-7662** is essentially an [TRC-721][trc721]/ERC-721 extension that adds an `IERC7662.getAgentData` view returning name, description, model, user/system prompt URIs, and a `promptsEncrypted` flag. It provides basic encryption status tracking, but the encryption itself is handled externally — the standard does not define how encrypted metadata is transferred or re-encrypted when ownership changes.
- **TRC-7857** goes further by building a full privacy-preserving transfer workflow into the standard itself. It defines on-chain verifier contracts and off-chain provers that cryptographically guarantee: (1) the seller actually holds the original data, (2) correct decryption and re-encryption occurred, (3) the new key is sealed with the buyer's public key, and (4) data availability is confirmed by the receiver's signature. It supports both TEE-based and ZKP-based verification paths.

In short, ERC-7662 handles "who owns the agent NFT and where the metadata lives," while TRC-7857 handles "how to securely transfer the actual agent intelligence (models, memory, character) without exposing it to anyone other than the rightful owner."

## Specification

This TIP defines three key interfaces: the main NFT interface, the metadata interface, and the data verification interface.
...
This standard is a pure smart contract specification. It can be implemented entirely on TVM without any protocol-level changes — all core operations (encrypted metadata hash storage, ownership tracking, and proof verification) are standard contract-level logic that TVM already supports. The heavy lifting (metadata encryption/re-encryption, AI model storage, oracle proof generation) all happens off-chain.

Cross-contract interactions follow the interfaces defined in this specification and rely on the existing cross-contract call mechanism that TVM shares with the EVM, with no protocol-layer modifications required:

1. The NFT contract calls the verifier contract through `verifier().verifyTransferValidity(...)` during `iTransfer` and `iClone`, consuming the structured `TransferValidityProofOutput[]` that the verifier returns.
2. The verifier contract queries the configured oracle / attestation contract (e.g., a TEE attestation contract) to validate the underlying proof material.
3. Interactions between the verifier contract and any oracle / attestation contract it relies on are implementation-specific and not standardized by this TIP; no additional coordination layer above the NFT–verifier interface is required.
...
This TIP does not inherit from existing NFT standards to maintain its focus on functional data management. However, implementations can choose to additionally implement [TRC-721][trc721] if traditional NFT compatibility is desired.

## V1 Scope

The immediate goal of this TIP is **specification only** — defining the interfaces, proof semantics, and transfer workflows adapted for TRON. Specifically:

- **In scope (normative).** The private-metadata NFT interface, the data verification interface, the ownership and transfer-validity proof semantics, and the transfer / clone / authorization workflows defined above.
- **Out of scope for V1.** Deployment of TEE/ZKP attestation contracts and prover networks, and any associated infrastructure roadmap. Such infrastructure belongs to the implementation layer and is tracked as a separate follow-up milestone.
- **TRC-721 compatibility.** Remains an optional recommendation, not a mandatory base, consistent with the equivalent treatment in [ERC-7857][erc7857].
- **Reference Implementation.** The contract code provided below is informative and intended to illustrate how the normative interfaces can be wired together; a production-grade reference implementation may follow once the spec is stable and the community has aligned on the verification approach.

## Implementation Notes

The TEE/ZKP re-encryption infrastructure required by this standard is specialized and beyond what general-purpose oracles (such as WINkLink) can provide. This TIP only standardizes the on-chain interfaces and proof semantics; deployment of the attestation contracts and prover networks is left to ecosystem participants. Possible paths include:

1. **Partnering with existing infrastructure providers** who already operate TEE/ZKP verification for [ERC-7857][erc7857] (for example, 0G documents a TEE-based oracle for ERC-7857 iNFT transfers).
2. **Community-driven development of TRON-native verification infrastructure**, in which case the attestation contracts targeted by the verifier are deployed on TVM directly.
3. **Cross-chain integration with existing off-chain prover networks**, in which case the on-chain verifier consumes attestations sourced from another network.

The choice between these paths is non-normative; any combination is acceptable as long as the verifier contract honors the interface and proof semantics defined in this TIP.

## Reference Implementation

*This section is informative / non-normative. The contracts below illustrate one possible wiring of the interfaces specified above and are not part of the normative requirements of this TIP.*
...
5. **Off-chain Storage**
    - This standard relies on privacy-preserving, verifiable off-chain storage for encrypted AI metadata; only data hashes and sealed keys are kept on-chain
    - The standard is orthogonal to the choice of storage backend and does not mandate a specific provider
    - On TRON, implementations MAY use [BTFS][btfs] or any other decentralized storage that offers comparable availability and integrity guarantees; cross-chain access to dedicated DA layers (for example, 0G Storage) is also an acceptable implementation path
    - Regardless of the backend, implementations should ensure that the ciphertext referenced by `dataHash` remains retrievable for as long as the corresponding token exists, and that retrieval does not leak the plaintext to unauthorized parties
...
[erc7662]: https://eips.ethereum.org/EIPS/eip-7662
[erc7857]: https://eips.ethereum.org/EIPS/eip-7857
[btfs]: https://www.btfs.io/
