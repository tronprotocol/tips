...
The following defines the  implementation of the shielded token contract. 
...
The `constructor` method binds the TRC-20 token address to the shielded token contract when it is deployed. The `scalingFactorLogarithm` is used as the scaling factor between the TRC-20 value and the shielded  token value, since the type of prior is `uint256` while the rear is `unit64`.  For examle, if   `scalingFactorLogarithm` is 2, it means 100 (10^2) TRC-20 token can be transformed to a shielded token.
...
```
    constructor (address trc20ContractAddress, uint256 scalingFactorLogarithm) public {
        ...
        scalingFactor = 10 ** scalingFactorLogarithm;
        owner = msg.sender;
...
```
    ...
    function mint(uint256 rawValue, bytes32[9] calldata output, bytes32[2] calldata bindingSignature, bytes32[21] calldata c) external {
        ...
        //step 4: store the note_commitment in the Merkle tree
        ...
        //step 5: store the latest root in the contract
        ...
        // step 7: transfer the TRC-20 Tokens from the sender to this contract
...
(2) Scale the TRC-20 value to shielded token value by `rawValueToValue` function. The `rawValue` must be the integer multiples of `scalingFactor`, which is initialised in the contructor function.
...
(3) verify the proof to check the validity of `note_commitment`by [verifyMintProof](https://github.com/tronprotocol/tips/blob/master/tip-137.md) function;

(4) store  the `note_commitment` in the merkle tree `tree`, which is a mapping data structure.

```
mapping(uint256 => bytes32) public tree;
```

(5) store the latest root in the `roots`, which is used for merkle path proof.

```
mapping(bytes32 => bytes32) public roots; 
```

(6) emit the related data ( `position`, `note_commitment`, `value_commitment`, `epk`, `c_enc`, `c_out`) as event to make it convenient for shielded note scanning.
...
(7) transfer the TRC-20 token to the shielded token contract by `transferFrom` function.
...
(3) verify the proof to check the validity of `note_commitment`by [verifyBurnProof](https://github.com/tronprotocol/tips/blob/master/tip-137.md)  instruction;

(4) store  the `nullifier` in the contract.

(5) transfer the TRC-20 token from the shielded token contract to `payTo` address by `transfer` function.

**getPath**

In order to make it convenient for users to construct zk-SNARK proof, the shielded token contract provides `getPath` function to return the latest root and merkle tree path for the given `note_commitment` with `position` parameter.
