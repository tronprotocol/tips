...
(1) transfer the TRC-20 token to the shielded contract by `transferFrom` function.

(2) check the `value` is positive and prevent duplicate note commitments.

(3) Scale the TRC-20 value to shielded token value by `rawValueToValue` function. The `rawValue` must be the integer multiples of `scalingFactor`, which is initialised in the contructor function.
