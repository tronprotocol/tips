...
(2) verify the proof to check the validity of `note_commitment`by [verifyMintProof](https://github.com/tronprotocol/tips/pull/133) function;
...
(5) emit the related data ( `position`, `note_commitment`, `value_commitment`, `c_enc`, `c_out`) as event to make it convenient for shielded note scanning.
...
(1) check the size of `input` and `output` to be in the range `[1,2]` ; check `nullifiers` to prevent the double spend. `nullifiers` is defined as a mapping structure in the contract. 

```
mapping(bytes32 => bytes32) public nullifiers; 
```

(2) verify the proof by [verifyTransferProof](https://github.com/tronprotocol/tips/pull/133)  function check the validity of the  `note_commitments`;
...
(2) verify the proof to check the validity of `note_commitment`by [verifyBurnProof](https://github.com/tronprotocol/tips/pull/133)  instruction;
...
For the technical details of Merkle tree implementation, please refer [timber](https://github.com/EYBlockchain/timber/blob/master/README.md).
