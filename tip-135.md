...
The `constructor` method binds the TRC-20 token address to the shielded TRC-20 contract when it is deployed. The `scalingFactorLogarithm` is used as the scaling factor between the TRC-20 value and the shielded  token value, since the type of prior is `uint256` while the rear is `unit64`.  For examle, if   `scalingFactorLogarithm` is 2, it means 100 (10^2) TRC-20 tokens can be transformed to a shielded token.
...
(3) verify the proof to check the validity of `note_commitment` by [verifyMintProof](https://github.com/tronprotocol/tips/blob/master/tip-137.md) function. Here, the input of the signed hash includes shielded contract address `this` , the scaled `value`, `output` and `c`. `output` includes `note_commitment`, `value_commitment`, `epk` and `zkproof`. `c` includes `c_enc` and `c_out`.
...
(7) transfer the TRC-20 token to the shielded contract by `transferFrom` function.
...
(3) verify the proof to check the validity of `note_commitment`by [verifyBurnProof](https://github.com/tronprotocol/tips/blob/master/tip-137.md)  instruction. The input of the signed hash includes shielded contract address `this` , `input`, `payTO` and the scaled `value`.  `input`  includes `nullifier`, `anchor`, `value_commitment`, `rk`, `zkproof`. `payTo` is the recipient address. 
...
(5) transfer the TRC-20 token from the shielded TRC-20 contract to `payTo` address by `transfer` function.
