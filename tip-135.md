...
(1) check the size of `input` and `output` to be in the range `[1,2]` ; check `nullifiers` to prevent the double spend. `nullifiers` is defined as a mapping structure in the contract. check the `notecommitment` to prevent the duplicate note commitments.
