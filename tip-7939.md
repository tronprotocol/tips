...
discussions-to: https://github.com/tronprotocol/tips/issues/838
status: Final
...
EIP-7939 introduces a new EVM opcode `CLZ` (`0x1e`) that counts the number of leading zero bits in a 256-bit stack value. This operation is a fundamental primitive natively supported by most modern processor architectures (x86, ARM), yet the EVM has lacked it since inception — forcing developers to use costly Solidity workarounds consuming ~184 gas and ~110–160 bytes of bytecode for what hardware accomplishes in a single cycle. The new opcode costs only 5 gas (matching `MUL`) and adds just 1 byte of bytecode, benefiting a wide range of use cases including fixed-point math (`lnWad`, `sqrt`, `powWad`), bitmap scanning, calldata compression, and post-quantum signature schemes. It also significantly reduces ZK proving costs by eliminating expensive dynamic bitwise shifts.

Bringing EIP-7939 to TRON would directly lower compute and deployment costs for on-chain math libraries, DeFi protocols, and ZK-related applications deployed on the TRON network. As `CLZ` is scheduled for Ethereum's Fusaka (Fulu/Osaka) hard fork, implementing it on TRON ensures continued EVM compatibility, reduces cross-chain development friction for Solidity developers targeting both ecosystems, and keeps TRON aligned with mainstream EVM opcode evolution. Therefore, EIP-7939 should be implemented on TRON as soon as possible to maintain compatibility with mainstream EVM standards.
