...
discussions-to: https://github.com/tronprotocol/tips/issues/845
status: Final
...
![Sample proxy on Etherscan](https://eips.ethereum.org/assets/eip-1967/Sample-proxy-on-etherscan.png)
...
The proposed storage slots for proxy-specific information are the following. More slots for additional information can be added in subsequent TRCs as needed.
...
### Recommended Proxy Detection Algorithm

Ecosystem tooling (block explorers, wallets, debuggers, and security analysis tools) is encouraged to implement slot-based proxy detection as the primary method, rather than relying on source code verification. This enables proxy identification for all contracts, including unverified ones.

The recommended detection algorithm is:

1. Read the implementation slot (`0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc`) via `eth_getStorageAt`.
2. Check that the returned value is a non-zero 20-byte address.
3. Verify that `getCode(address)` at the resolved address returns non-empty bytecode.
4. If the implementation slot is empty, read the beacon slot (`0xa3f0ad74e5423aebfd80d3ef4346578335a9a72aeaee59ff6cb3582b35133d50`) and repeat steps 2-3 on the beacon address. If valid, call `implementation()` on the beacon contract to resolve the final logic address.
5. Optionally, read the admin slot (`0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103`) to classify the proxy type (Transparent proxy vs. UUPS vs. Beacon proxy).

Step 3 is critical for avoiding false positives - a valid address at the implementation slot alone is insufficient, as any contract could incidentally write a valid address to that slot.

> **Note (TRON/TVM):** Although TRON uses base58check encoding for addresses externally, addresses stored in these standardized slots follow the EVM-compatible 20-byte hex format. Tooling that reads these slots via `eth_getStorageAt` will receive standard 32-byte hex values (with the address left-padded to 32 bytes). Developers coming from a pure TRON background should be aware that no base58 conversion is involved at the storage level.

## Rationale

This TIP standardises the **storage slot** for the logic contract address, instead of a public method on the proxy contract. The rationale for this is that proxies should never expose functions to end users that could potentially clash with those of the logic contract.
 ...
 * https://github.com/tronprotocol/tips/issues/845[TIP1967], so that it doesn't conflict with the storage layout of the
 * implementation behind the proxy.
 */
contract TRC1967Proxy is Proxy, TRC1967Upgrade {
    /**
    ...
    function _implementation() internal view virtual override returns (address impl) {
        return TRC1967Upgrade._getImplementation();
    }
}

/**
 * @dev This abstract contract provides getters and event emitting update functions for
 * https://github.com/tronprotocol/tips/issues/845[TIP1967] slots.
 */
abstract contract TRC1967Upgrade {
    ...
    /**
     * @dev Stores a new address in the TIP1967 implementation slot.
     */
    function _setImplementation(address newImplementation) private {
        require(Address.isContract(newImplementation), "TRC1967: new implementation is not a contract");
    ...
    function _upgradeToAndCallSecure(
        ...
        if (!rollbackTesting.value) {
            ...
            require(oldImplementation == _getImplementation(), "TRC1967Upgrade: upgrade breaks further upgrades");
    ...
    /**
     * @dev Stores a new address in the TIP1967 admin slot.
     */
    function _setAdmin(address newAdmin) private {
        require(newAdmin != address(0), "TRC1967: new admin is the zero address");
    ...
    /**
     * @dev Stores a new beacon in the TIP1967 beacon slot.
     */
    function _setBeacon(address newBeacon) private {
        require(Address.isContract(newBeacon), "TRC1967: new beacon is not a contract");
        require(
            Address.isContract(IBeacon(newBeacon).implementation()),
            "TRC1967: beacon implementation is not a contract"
        );
...
This TRC relies on the fact that the chosen storage slots are **not** to be allocated by the solidity compiler. This guarantees that an implementation contract will not accidentally overwrite any of the information required for the proxy to operate. As such, locations with a high slot number were chosen to avoid clashes with the slots allocated by the compiler. Also, locations with no known preimage were picked, to ensure that a write to mapping with a maliciously crafted key could not overwrite it.
