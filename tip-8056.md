...
- prevent multiplication overflow or revert explicitly before returning an incorrect scaled amount.

If the schedule is active when `block.timestamp >= effectiveAt`, a setter that seals the active value before scheduling the next one MUST also use `>=`, not `>`. Otherwise an update in the exact effective block can temporarily restore the old multiplier.
...
Production implementations MUST add appropriate access control and overflow-safe conversion. The example fixes the exact-equality state-transition inconsistency present in early reference code.
...
7. overflow rejection at large raw amounts or multipliers; and
