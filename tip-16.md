...
**Scenario 3** for active-permission:
...
**Scenario 4** for witness-permission:
...
(Current Scenario) \
Alice can assign the witness-permission to the administrator. Since the administrator only has the producing-block permission, there is no TRX transfer permission, and even if the private key of the administrator on the server is compromised, TRX will not be lost.
