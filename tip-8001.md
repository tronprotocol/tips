...
discussions-to: https://github.com/tronprotocol/tips/issues/828
status: Final
...
TRC-8001 defines a minimal, single-chain primitive for **multi-party agent coordination**. An initiator posts an intent and each participant provides a verifiable acceptance attestation. Once the required set of acceptances is present and fresh, the intent is executable. The standard specifies typed data, lifecycle, mandatory events, and verification rules compatible with [EIP-712], [ERC-1271], [EIP-2098], and [EIP-5267].

TRC-8001 omits privacy, reputation, threshold policies, bonding, and cross-chain semantics. Those are expected as optional modules that reference this specification.

## Motivation

Agents in DeFi/MEV/Web3 Gaming and Agentic Commerce often need to act together without a trusted coordinator. Existing intent standards (e.g., [ERC-7521](https://eips.ethereum.org/EIPS/eip-7521), [ERC-7683](https://eips.ethereum.org/EIPS/eip-7683)) define single-initiator flows and do not specify multi-party agreement.

TRC-8001 specifies the smallest on-chain primitive for that gap: an initiator's [EIP-712](https://eips.ethereum.org/EIPS/eip-712) intent plus per-participant [EIP-712](https://eips.ethereum.org/EIPS/eip-712)/[EIP-1271](https://eips.ethereum.org/EIPS/eip-1271) acceptances. The intent becomes executable only when the required set of acceptances is present and unexpired. Canonical (sorted-unique) participant lists and standard typed data provide replay safety and wallet compatibility. Privacy, thresholds, bonding, and cross-chain are left to modules.
...
Implementations SHOULD expose the domain via [ERC-5267](https://eips.ethereum.org/EIPS/eip-5267).
...
- the signature does not validate the supplied `AgentIntent` under the TRC-8001 EIP-712 domain;
...
- the attestation signature does not validate under the TRC-8001 domain;
...
- Keeping TRC-8001 single-chain avoids coupling to bridge semantics and keeps the primitive audit-friendly.
- Wallet friendliness: EIP-712 arrays let signers see actual participant addresses.

### Reference Scenario

One natural fit for TRON is multi-agent DeFi coordination — for example, agents collaborating on cross-DEX execution via on-chain intents, with atomic settlement and result verification handled by the coordination framework.

## TVM Compatibility

> **Note (TRON/TVM):** TRC-8001 targets the TRON Virtual Machine and is fully compatible with EVM-style EIP-712 signing primitives used by TRON wallets.
>
> - **`chainId`**: follows [TIP-712](https://github.com/tronprotocol/tips/blob/master/tip-712.md), which defines `chainId` as `block.chainid & 0xffffffff` on TVM (e.g. `728126428` on TRON mainnet). The same convention applies on Nile and Shasta testnets.
> - **`address`**: within EIP-712 typed-data encoding, addresses MUST be encoded as `uint160` after removing the TRON-specific `0x41` prefix byte, as defined in [TIP-712](https://github.com/tronprotocol/tips/blob/master/tip-712.md). The `participants` array and all address fields follow this form.
> - **Signatures**: ECDSA over secp256k1 with low-`s` enforcement is the canonical form and is wire-compatible with TRON wallet signatures. Both 65-byte and [EIP-2098](https://eips.ethereum.org/EIPS/eip-2098) 64-byte compact forms MUST be accepted, alongside [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) contract-wallet signatures.

## Backwards Compatibility

TRC-8001 introduces a new interface. It is compatible with EOA and contract wallets via ECDSA and ERC-1271. It does not modify existing standards.

### Related TIPs

TRC-8001 is the coordination primitive. [TRC-1271](https://github.com/tronprotocol/tips/blob/master/tip-1271.md) is used for agent signature validation within the coordination flow, and [TRC-8126](https://github.com/tronprotocol/tips/issues/832) (proposed) is intended to provide the complementary agent registration and verification layer.

## Reference Implementation

A permissive reference implementation is provided in [`contracts/AgentCoordination.sol`](https://github.com/ethereum/ERCs/blob/master/assets/erc-8001/contracts/AgentCoordination.sol). It uses a minimal ECDSA helper and supports ERC-1271 signers. It enforces participant canonicalisation, intent nonces, acceptance freshness, and all-participants policy.
