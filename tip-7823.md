...
discussions-to: https://github.com/tronprotocol/tips/issues/826
status: Final
...
EIP-7823 introduces a clear upper bound (8192 bits per parameter) on the input sizes of the MODEXP precompiled contract, restricting its previously unbounded input space. This reduces potential consensus risks, improves execution determinism, and makes the contract easier to test and maintain. The change has virtually no impact on existing applications but significantly enhances protocol-level safety and stability.

Bringing EIP-7823 to TRON would improve alignment with Ethereum's Osaka upgrade, strengthen predictability for cryptographic workloads relying on MODEXP, and reduce cross-chain development friction. Therefore, EIP-7823 should be implemented on TRON as soon as possible to maintain compatibility with mainstream EVM standards.
